...
func _process(delta):
	secret_path()
	#print ($ElfBow2.isDead)
	#func dead_enemies():
	#disable_movement(Goblin)
	#enable_movement(Goblin)
	pass
...
func disable_movement (enemy):
	...
	$Goblin/CollisionShape2D.set_deferred("disabled", true)
	$Goblin/HitBox.monitoring = false
	$Goblin/EnemyHitBox.monitoring = false
	$Goblin/PlayerDetector.monitoring = false
	
func enable_movement (enemy):
	enemy.visible = true
	enemy.speed = Goblin_speed
	enemy.z_index = Goblin_z_index
	enemy.gravity = Goblin_gravity
	enemy.isDead = false
	enemy.set_process(true)
	#$Goblin/CollisionShape2D.set_deferred("disabled", false)
	$Goblin/HitBox.monitoring = true
	$Goblin/EnemyHitBox.monitoring = true
	$Goblin/PlayerDetector.monitoring = true
"""
	var enemy_1 = goblin.instance()
	enemy_1.position = $Positions/PositionGob1.position
	var enemy_2 = goblin.instance()
	enemy_2.position = $Positions/PositionGob2.position
	if spawn_enemy == true:
		var goblin_1 = add_child(enemy_1)
		var goblin_2 = add_child(enemy_2)
		spawn_enemy = false
"""
...
func unused_code():
	var elfBow = ElfBow.instance()
	elfBow.is_moving_right = true
	elfBow.position = $Positions/PositionElfBow.position
	add_child(elfBow)
	elfBow.airSpawn = false
	elfBow.speed = 0
